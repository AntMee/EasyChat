...
/// <summary>
/// Windows clipboard snapshot adapter. The selection workflow depends only on
/// <see cref="IClipboardSnapshotService"/> so other platforms can provide their
/// own implementation without referencing Win32 or OLE. The adapter itself does
/// not touch Avalonia or the UI thread: materialized Win32 formats are copied as
/// HGLOBAL bytes and OLE providers stay on the dedicated worker process/STA.
/// </summary>
[SupportedOSPlatform("windows")]
public sealed class WindowsClipboardProcessClient : IClipboardSnapshotService
{
    ...
    public void Restore(IClipboardSnapshot? snapshot, ILogger? logger = null)
    {
        ...
        if (snapshot is not ClipboardSnapshot windowsSnapshot)
        {
            snapshot.Dispose();
            return;
        }
        ...
        try
        {
            Worker.Value.Restore(windowsSnapshot.Token, logger);
        }
        catch (Exception ex)
        {
    ...
    public uint GetChangeToken() => GetClipboardSequenceNumberNative();
