...
public sealed class WindowsScreenCaptureTests
{
    ...
    [
        ...
        new("display-above", new PhysicalScreenRegion(0, -1200, 1920, 1200), 144, 144, false),
    ...
    public async Task CaptureAsync_ResolvesPrimaryStableIdAndPhysicalRegion()
    {
        ...
        Assert.AreEqual(Screens[2].Bounds, backend.CapturedRegion);
    ...
    [TestMethod]
    public async Task CaptureAsync_PreservesCrossScreenRegionWithNegativeCoordinates()
    {
        var backend = new FakeScreenBackend(Screens, new byte[8]);
        var capture = new WindowsScreenCapture(backend);
        var region = new PhysicalScreenRegion(-320, -240, 960, 720);

        var result = await capture.CaptureAsync(new ScreenCaptureRequest(
            ScreenCaptureTarget.Region,
            Region: region));
    ...
    public async Task ScreenCatalog_PreservesBoundsDpiAndPrimaryFlag()
    {
        ...
        Assert.AreEqual(new PhysicalScreenRegion(0, -1200, 1920, 1200), result[1].Bounds);
        Assert.AreEqual(144d, result[1].DpiY);
        Assert.AreEqual(120d, result[2].DpiX);
        Assert.IsTrue(result[2].IsPrimary);
    }
